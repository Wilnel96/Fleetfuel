...
type Phase = 'selection' | 'review' | 'history' | 'history-detail';
...
export default function DebitOrderRun({ onBack }: { onBack: () => void }) {
  ...
  // Excluded invoices (deselected by operator); credit notes selected (opt-in)
  ...
  const loadData = async () => {
    try {
      ...
      for (const inv of debitInvoices) {
        ...
        if (!grouped[orgId]) {
          ...
          grouped[orgId] = {
            ...
            invoices: [],
            credit_notes: creditsByOrg[orgId] || [],
            total_outstanding: 0,
            total_credits: (creditsByOrg[orgId] || []).reduce((s, c) => s + c.total_amount, 0),
          };
        }
      ...
      setExcludedInvoices(new Set()); // all invoices start included
    } catch (err: any) {
  ...
  const toggleCreditNote = (cnId: string) => {
    const next = new Set(selectedCreditNotes);
  ...
  const creditsAmount = allCreditNotes
    .filter(cn => selectedCreditNotes.has(cn.id))
    .reduce((s, cn) => s + cn.total_amount, 0);
  const netAmount = Math.max(0, grossAmount - creditsAmount);

  const missingBankingOrgs = orgGroups.filter(g => !g.banking_complete && g.invoices.some(inv => !excludedInvoices.has(inv.id)));
  const missingBankingInvoiceIds = new Set(missingBankingOrgs.flatMap(g => g.invoices.map(i => i.id)));

  // Effective included invoices for bank file — must have banking details and not excluded
  ...
  const processRun = async () => {
    ...
    try {
      ...
      const appliedCreditNotes = allCreditNotes.filter(cn => selectedCreditNotes.has(cn.id));
      ...
      // Insert run items for ALL invoices (included and excluded)
      const runItems = allInvoices.map(inv => ({
        ...
        credits_amount: 0,
        net_amount: excludedInvoices.has(inv.id) ? 0 : inv.amount_outstanding,
        status: excludedInvoices.has(inv.id) ? 'excluded' : 'included',
        exclusion_reason: excludedInvoices.has(inv.id) ? (exclusionReasons[inv.id] || null) : null,
      }));
  ...
  const redownloadBankFile = async (run: HistoryRun) => {
    ...
    // Build minimal invoice list from history items
    ...
    <div className="mb-8">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={phase === 'review' ? () => setPhase('selection') : phase === 'history' || phase === 'history-detail' ? () => setPhase('selection') : onBack}
            className="flex items-center gap-2 px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors text-sm font-medium"
          >
            <ArrowLeft className="h-4 w-4" />
            {phase === 'review' ? 'Back to Selection' : phase === 'history' || phase === 'history-detail' ? 'Back to Run' : 'Back to Invoices'}
          </button>
        </div>
      </div>
                  ...
                  <div className="divide-y divide-gray-100">
                    {included.map(item => (
                      <div key={item.id} className="px-6 py-3 grid grid-cols-3 gap-4 text-sm">
                        ...
                        <span className="text-right font-semibold text-gray-900">{formatCurrency(item.amount)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {historyRuns.map(run => (
                    <tr key={run.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-3 text-right font-semibold text-gray-900">{formatCurrency(run.net_amount)}</td>
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-5xl mx-auto">
          {pageHeader('Review Debit Order Run', 'Confirm the details below before processing')}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Included', value: includedInvoices.length, sub: 'invoices', color: 'text-green-700', bg: 'bg-green-50 border-green-200' },
              { label: 'Excluded', value: excludedInvoiceList.length, sub: 'invoices', color: 'text-red-700', bg: 'bg-red-50 border-red-200' },
              { label: 'Gross Amount', value: formatCurrency(grossAmount), sub: 'before credits', color: 'text-gray-900', bg: 'bg-white border-gray-200' },
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-600 mt-2">
                    These invoices will be marked as paid in the system but excluded from the CSV bank submission file.
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. May 2026 Debit Order Run"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                />
              </div>
            </div>
                ...
                return (
                  <div key={inv.id} className="px-6 py-2.5 flex items-center justify-between text-sm">
                    <div>
                      ...
                      {!hasBanking && (
                        <span className="ml-2 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">No banking details</span>
                      )}
                    </div>
                    <span className="font-semibold text-gray-900">{formatCurrency(inv.amount_outstanding)}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
              <div className="bg-red-50 px-6 py-3 border-b border-red-200 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-red-700">Excluded / Failed ({excludedInvoiceList.length})</h3>
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        {pageHeader(
          'Debit Order Run',
          'All debit order clients are pre-selected. Untick any failed or excluded debit orders before processing.'
        )}
        ...
        {orgGroups.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 text-lg font-medium">No outstanding invoices for debit order clients</p>
          ...
          <>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4 flex flex-wrap items-center gap-4 justify-between">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="text-right">
                  <p className="text-xs text-gray-500 uppercase tracking-wide">Gross Amount</p>
            ...
            {missingBankingOrgs.length > 0 && (
              <div className="mb-4 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-center gap-3">
                ...
                <p className="text-sm text-amber-800">
                  <span className="font-semibold">{missingBankingOrgs.length} organisation(s)</span> are missing banking details — they will be excluded from the bank submission file. Please update their financial information.
                </p>
              </div>
            )}
            ...
            <div className="space-y-3">
              {filteredGroups.map(group => {
                ...
                const orgNetOutstanding = group.invoices
                    ...
                    <div className="px-5 py-4 flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          ...
                          <p className={`text-base font-bold ${allIncluded ? 'text-gray-900' : 'text-red-500'}`}>
                            {formatCurrency(orgNetOutstanding)}
                          </p>
                        </div>
                        {group.credit_notes.length > 0 && (
                          <div className="text-right">
                            ...
                            <p className="text-sm font-semibold text-green-700">
                              {group.credit_notes.length} available
                            </p>
                          </div>
                        )}
                        <button
                      ...
                      <>
                        <div className="border-t border-gray-100 divide-y divide-gray-50">
                          {group.invoices.map(inv => {
                            ...
                            const isOverdue = inv.status === 'overdue' || (inv.payment_due_date && new Date(inv.payment_due_date) < new Date() && inv.status === 'issued');
                            return (
                              <div
                                key={inv.id}
                          ...
                          <div className="border-t-2 border-green-200 bg-green-50 divide-y divide-green-100">
                            <div className="px-6 py-2 flex items-center gap-2">
                              ...
                              <p className="text-xs font-semibold text-green-800 uppercase tracking-wide">
                                Credit Notes — tick to apply this month
                              </p>
                            </div>
                            {group.credit_notes.map(cn => {
                              const isSelected = selectedCreditNotes.has(cn.id);
                              return (
                                <div key={cn.id} className={`px-6 py-3 flex items-center justify-between transition-colors ${isSelected ? 'bg-green-100' : ''}`}>
                                  <div className="flex items-center gap-4">
                                    <input
                                      type="checkbox"
                                      checked={isSelected}
                                  ...
                                  <div className="text-right">
                                    ...
                                    <p className="text-xs text-gray-400">{isSelected ? 'Will be applied' : 'Not applied'}</p>
                                  </div>
                                </div>
                              );
                            })}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-blue-800">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-blue-700">
                    <li>Only organisations with payment method set to "Debit Order" are shown</li>
                    <li>All invoices are pre-selected — untick only the ones that should not be debited</li>
                    <li>Expand an organisation row to view individual invoices and optionally note exclusion reasons</li>
                    <li>Credit notes are opt-in — tick those you want to apply this month</li>
                    <li>A bank submission CSV file is automatically downloaded when the run is processed</li>
                    <li>Organisations missing banking details are included in the system but excluded from the CSV file</li>
                  </ul>
                </div>
              </div>
            </div>
